...
1. **Логотип и статус связи**:
   ...
   - `⚡ 12 ms` — динамический индикатор сетевой задержки (Health Check Ping), непрерывно отслеживающий отклик API и доступность БД.
...
3. **Уведомления Telegram и мониторинг (`TELEGRAM ALERTING`)**: настройка бота и чата для моментального оповещения об ошибках и сбоях.
4. **Журнал аудита безопасности (`SECURITY AUDIT TRAIL`)**: протоколирование действий диспетчеров и событий безопасности.
5. **Диагностика системы и БД (`SYSTEM HEALTH & DIAGNOSTICS`)**: аппаратные метрики, объемы таблиц и статус сервера.
6. **Управление учетными записями (`RBAC`)**: создание диспетчеров и выдача паролей.
7. **Автоматические резервные копии (30 дней)**: создание и восстановление бэкапов.
8. **База данных рейсов**: журнал всех вылетов за последние 10 дней с поиском и удалением.
...
## 12. Прямая синхронизация с системой регистрации Lydia DCS

Для моментального обновления базы закрытых рейсов без необходимости ручной выгрузки Excel-файлов предусмотрен модуль прямой синхронизации с **Lydia DCS**:

![Рисунок 9. Модальное окно синхронизации с Lydia DCS](manual_screenshots/fig9_dcs_sync_modal.png)

### Порядок работы:
1. Во вкладке **«Администрирование»** нажмите кнопку **`[ 📡 Синхронизация с Lydia DCS ]`**.
2. В появившемся окне укажите:
   - **Дату вылета** (или выберите быстрый пресет: `Вчера`, `Сегодня`, `7 дней`);
   - **Авиакомпанию** (`N4` / `EO`);
   - Выберите интересующие **станции вылета** (например, `AER`, `SVO`, `KZN`, `LED`...).
3. Нажмите **`[ 🔍 Сканировать Lydia DCS ]`** — система в реальном времени обратится к API DCS и отобразит список закрытых рейсов с пассажирами и весом багажа.
4. Отметьте нужные рейсы галочками и нажмите **`[ 📥 Импортировать в базу ]`**.

---

## 13. Пополнение базы данных и выгрузка из Astra DCS
...
## 14. Модуль исторического бэктестинга (Проверка точности)
...
## 15. Настройка Telegram-оповещений и мониторинга (TELEGRAM ALERTING)

Система поддерживает моментальную доставку критических предупреждений и системных ошибок в Telegram-чат или канал администратора/ИТ-отдела.

### Пошаговая настройка бота:
1. **Создание бота**:
   - В Telegram откройте бота [`@BotFather`](https://t.me/BotFather) и отправьте команду `/newbot`.
   - Задайте имя (например, `AeroBag Alerts`) и логин бота (например, `aerobag_alert_bot`).
   - Скопируйте полученный **HTTP API Token** (формат: `123456789:ABCdefGHIjklMNOpqrSTUvwxYZ`).
2. **Получение Chat ID**:
   - Добавьте созданного бота в ваш канал/группу или начните с ним личный диалог, нажав `/start`.
   - Чтобы узнать ID чата, перешлите любое сообщение боту [`@userinfobot`](https://t.me/userinfobot) или [`@getmyid_bot`](https://t.me/getmyid_bot).
   - Скопируйте числовой `Id` (для личного чата: `123456789`, для супергруппы: `-1001234567890`).
3. **Ввод данных в AeroBag Predictor**:
   - Перейдите во вкладку **«Администрирование»** $\rightarrow$ раскрывающийся блок **«3. Уведомления Telegram и мониторинг»**.
   - Вставьте **Токен бота (Bot Token)** и **ID чата (Chat ID)** в соответствующие поля ввода.
   - Используйте кнопку `👁️` / `🙈` для безопасного просмотра или скрытия токена.
   - Убедитесь, что установлена галочка **«Включить отправку алертов об ошибках в Telegram»**.
   - Нажмите **`[ 🧪 Тест соединения ]`** — бот мгновенно отправит тестовое сообщение `🚀 ТЕСТОВОЕ СООБЩЕНИЕ AEROBAG PREDICTOR`.
   - Нажмите **`[ 💾 Сохранить настройки Telegram ]`**.

> [!NOTE]
> В систему встроен механизм защиты от флуда (Anti-Flood Protection): одинаковые ошибки дедуплицируются с таймаутом 180 секунд, предотвращая блокировку бота при циклических сбоях.

---

## 16. Журнал аудита безопасности (SECURITY AUDIT TRAIL)

Раздел **«4. Журнал аудита безопасности и действий пользователей»** во вкладке «Администрирование» фиксирует все значимые события в системе:

* 🔐 **Авторизация**: успешные входы (`AUTH_LOGIN`) и неудачные попытки с неверным паролем (`AUTH_FAILED`).
* 👥 **Управление доступом**: создание пользователей (`USER_CREATE`), редактирование ролей (`USER_UPDATE`), сброс паролей (`PASSWORD_CHANGE`), удаление учетных записей (`USER_DELETE`).
* ✈️ **Операции с рейсами**: сохранение и импорт статистики (`FLIGHTS_SAVED`), ручное удаление рейсов (`FLIGHT_DELETE`), очистка базы (`DB_CLEARED`).
* 💾 **Резервное копирование**: создание бэкапов (`BACKUP_CREATE`), скачивание и откат базы (`BACKUP_RESTORE`).
* ⚙️ **Системные настройки**: сохранение конфигурации Telegram (`SETTINGS_TELEGRAM_UPDATE`) и глобальных параметров (`SETTINGS_UPDATE`).

### Возможности журнала:
1. **Фильтрация по категориям**: выпадающий список позволяет мгновенно отфильтровать записи по типу (`Авторизация`, `Пользователи`, `Рейсы`, `Бэкапы`, `Настройки`, `Ошибки`).
2. **Детализация**: фиксируется точное время (UTC+3), логин инициатора, IP-адрес, тип события и подробные параметры.
3. **Авторотация (90 дней)**: события старше 90 дней архивируются автоматически для поддержания высокой производительности базы данных.

---

## 17. Диагностика системы и БД (SYSTEM HEALTH & DIAGNOSTICS)

Раздел **«5. Диагностика системы и базы данных»** предоставляет актуальную телеметрию работоспособности серверного окружения:

* 💾 **Размер базы данных**: суммарный объем таблиц `flights`, `users`, `audit_logs` и служебных индексов в мегабайтах (МБ).
* 🗄️ **Объем архивов бэкапов**: занятое дисковое пространство папкой резервных копий `backups/`.
* 📅 **Временной диапазон истории**: точные даты самого первого и самого последнего рейса в базе.
* 🏢 **Статистика по авиакомпаниям**: количество рейсов в разрезе каждого перевозчика (`N4`, `EO` и др.).
* ⚙️ **Окружение сервера**: версия PHP, веб-сервера Apache, версия MySQL, лимит памяти (`memory_limit`) и права на запись в служебные каталоги (`backups/`, `logs/`).
